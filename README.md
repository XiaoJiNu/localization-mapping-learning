...
| 单元 | 主题 | 预计用时 | 状态 | 主要产出 |
|---:|---|---:|---|---|
| 0 | 基础诊断与能力画像 | — | 已完成 | [第一次诊断复盘](articles/00_diagnostic_review.md) |
| 1 | 坐标系与坐标链 | $2\,\mathrm{h}$ | 进行中 | 笔记、推导、测验、变换实验 |
| 2 | 旋转与刚体位姿 | $2\,\mathrm{h}$ | 待开始 | 旋转表示与数值实验 |
| 3 | 传感器、时间与标定 | $2\,\mathrm{h}$ | 待开始 | 时间线、外参和运动补偿实验 |
| 4 | 最小二乘基础 | $2\,\mathrm{h}$ | 待开始 | 投影、秩与加权最小二乘实验 |
| 5 | 概率与卡尔曼更新 | $2\,\mathrm{h}$ | 待开始 | 一维融合与协方差实验 |
| 6 | IMU 与轮式推算 | $2\,\mathrm{h}$ | 待开始 | 比力、重力、零偏和积分实验 |
| 7 | 局部里程计 | $2\,\mathrm{h}$ | 待开始 | 多种里程计对比 |
| 8 | 滤波与因子图 | $2\,\mathrm{h}$ | 待开始 | 递推估计与批量优化对比 |
| 9 | SLAM 与全局修正 | $2\,\mathrm{h}$ | 待开始 | 回环、图优化与 TF 分析 |
| 10 | 重定位与综合设计 | $2\,\mathrm{h}$ | 待开始 | 场景方案设计与评审 |
...
- 右手系；
- 列向量与左乘；
- ${}^{A}\mathbf T_B$ 把在坐标系 $B$ 中表示的量映射到坐标系 $A$；
- 齐次变换写为

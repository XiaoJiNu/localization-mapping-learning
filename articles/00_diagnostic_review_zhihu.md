...
<!--
知乎发布备注（不属于正文，发布时不要复制）：

1. articles/assets/diagnostic_review/diagnostic_review_cover.png
2. articles/assets/diagnostic_review/diagnostic_review_actual_path.png
3. articles/assets/diagnostic_review/diagnostic_review_false_mastery.png
4. articles/assets/diagnostic_review/diagnostic_review_ability_profile.png

请在知乎编辑器中按正文出现顺序重新上传图片，不要直接复制仓库相对路径。
块公式需要在知乎公式编辑器中逐项确认，发布前使用手机宽度预览。
-->
...
题目给出均值为 10、方差为 4 的一维先验，以及观测值 12：

$$
x\sim\mathcal{N}(10,4),\qquad z=12
$$

测量噪声方差为 1。ChatGPT 先问融合结果会更接近 10 还是 12。
...
后续追问又暴露出一串关联问题：静止时，加速度计仍会测到大小约为重力加速度 g 的比力；姿态有小误差时，重力会被错误投影到水平方向，虚假加速度经过积分形成速度和位置漂移；匀速转弯虽然速度大小不变，方向却在变，因此仍有向心加速度。
...
模块评分与主要缺口保存在[第一次诊断报告](https://github.com/XiaoJiNu/localization-mapping-learning/blob/main/docs/00_learning_system/diagnostic_report.md)中，错误索引作为配套文档保存在同一目录。
...
20 小时不是“学会全部 SLAM”的承诺。第一轮目标只是补稳第二级、稳定第三级，并开始积累第四级的实验与故障分析证据。每个单元都要留下“诊断—学习—推导—实现—复盘—输出”的证据链，再把核心概念、公式和常见错误压缩成一张知识卡。完整安排见[学习地图](https://github.com/XiaoJiNu/localization-mapping-learning/blob/main/docs/00_learning_system/learning_map.md)，进度记录保存在同一目录。

第三类是五个核心资源：Strang 的 *Introduction to Linear Algebra*、Barfoot 的 *State Estimation for Robotics*、ROS tf2 与 REP-105、GTSAM 文档、RTAB-Map 文档。它们是遇到问题时查阅的事实锚点，不是要求从头到尾同时通读的书单，也不能被 AI 生成内容替代。资料入口见[阅读清单](https://github.com/XiaoJiNu/localization-mapping-learning/blob/main/references/reading_list.md)。
...
首个[坐标变换链最小实验](https://github.com/XiaoJiNu/localization-mapping-learning/blob/main/experiments/exp_001_transform_chain/README.md)已经建立并跑通。它只验证一件事：点从 `base` 到 `odom`、再到 `map` 的逐步变换，与先复合两个变换再直接计算，结果在浮点误差内一致。
...
> 本文是面向知乎重新整理的版本。更完整的原始问答、公式推导和九模块评分见[原版诊断复盘](https://github.com/XiaoJiNu/localization-mapping-learning/blob/main/articles/00_diagnostic_review.md)。

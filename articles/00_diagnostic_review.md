...
我没有把原文的六类提示词一次性丢给 ChatGPT，而是按“定义目标—闭卷诊断—整理结论—生成证据”执行。

第一步，我按上述规则做闭卷诊断。我不查资料，直接写下结论、推理和不确定之处；ChatGPT 再标出答对、遗漏和错误的部分，只讲错处。这样留下的不是听完讲解后的标准答案，而是我的能力边界。IMU 部分追问过多后，我又把规则改为每个模块最多一道主问题和一道必要追问。

第二步，诊断结束后，我才让 ChatGPT 汇总结果：先形成[诊断报告](../docs/00_learning_system/diagnostic_report.md)和[错误索引](../docs/00_learning_system/error_index.md)，再据此生成[五级学习阶梯与 20 小时学习地图](../docs/00_learning_system/learning_map.md)、[进度表](../docs/00_learning_system/progress.md)和[复习计划](../docs/00_learning_system/review_schedule.md)，并筛出[核心资源](../references/reading_list.md)。

第三步，我把这些产物组织进公开单体仓库，避免它们随聊天结束而消失：`docs/` 保存规则、笔记、推导、测验和错题；`experiments/` 保存实验；`src/` 与 `tests/` 保存最小实现和验证；`references/` 保存资料索引；`articles/` 保存阶段复盘。第一个[坐标变换链实验](../experiments/exp_001_transform_chain/README.md)已经用代码验证逐步变换与复合变换结果一致。

于是，一次对话变成了一条可追踪的证据链：原始回答暴露问题，诊断形成能力画像，能力画像决定学习顺序，知识再通过推导、测验、代码和延迟复习接受验证。
...
- 不知道求梯度时为什么会出现矩阵 $\mathbf{A}^{\mathsf{T}}$；
- 求解过程中算出了逆矩阵，却漏乘右侧向量；
...
这表示残差与矩阵 $\mathbf{A}$ 的列空间正交，而 $\mathbf{A}\hat{\mathbf{x}}$ 是 $\mathbf{b}$ 在该列空间上的正交投影。
...
题目没有指定物理单位，因此这里把变量 $x$ 和后面的 $z$ 视为使用同一抽象长度单位；正态分布的第二个参数 4 表示方差，单位是该抽象长度单位的平方。
...
传感器题问 IMU 和轮速计直接测量什么。我最初回答，IMU 测量车辆的姿态，甚至把位置变量 $x$ 和 $y$ 也放了进去；轮速计测量车轮转速。

这里包含两个问题：变量 $x$ 和 $y$ 表示位置，不是姿态；更重要的是，IMU 并不直接测量位置和姿态。它通常直接测量：
...
问题最集中出现在 IMU 模块：ChatGPT 根据回答连续追问，诊断逐渐变成教学，题量和时间失控。我发现后把规则改为“每模块最多一道主问题和一道必要追问；超出内容进入后续学习清单”。

这次实践还得到三条边界：

- **评分只看相对强弱**。7 分或 8 分不是标准化结论；后续统一按核心结论、推理、公式与单位、工程联系评分。
- **当场听懂不等于掌握**。必须用第 1、3、7、14 天延迟测试、手算或最小实验，以及项目故障分析复验。
- **AI 不是唯一事实来源**。公式、坐标约定、ROS 行为和算法结论仍需教材、官方文档、论文、代码与实验交叉验证。
...
这张图不能只停留在箭头上。本次诊断已形成以下结论和仓库产物：

| 环节 | 本次诊断后的结论 | 仓库索引 |
|---|---|---|
| 目标 | 把“学会”拆成画系统、讲算法、用公式、跑实验和做工程判断五种可验证能力 | [README：五种能力](../README.md#我希望练成的五种能力) |
| 诊断 | 闭卷检查九个模块；今后每模块最多一题一追问，只寻找能力边界 | [第一次诊断报告](../docs/00_learning_system/diagnostic_report.md) |
| 能力画像 | 系统结构与工程直觉较好；数学估计、IMU、滤波优化和时间细节较弱 | [诊断结论](../docs/00_learning_system/diagnostic_report.md#结论)、[错误索引](../docs/00_learning_system/error_index.md) |
| 学习阶梯 | 按“完全新手—基本理解—实用使用者—问题解决者—自信的实践者”逐级取证，而不是听懂后直接升级 | [五级学习阶梯](../docs/00_learning_system/learning_map.md#五级学习阶梯) |
| 核心资源 | 第一轮只用 Strang、Barfoot、ROS 坐标与时间文档、GTSAM、RTAB-Map | [第一轮核心资源](../references/reading_list.md#第一轮核心资源)、[公开项目索引](../references/repositories.md) |
| 20 小时第一轮 | 十个两小时单元：前六个补短板，后四个建闭环，不追求“学完 SLAM” | [学习地图](../docs/00_learning_system/learning_map.md)、[学习进度](../docs/00_learning_system/progress.md) |

我的等级并不整齐：里程计、SLAM 和重定位的系统关系已部分达到第三级；线性代数、概率估计和 IMU 仍需补齐第一、二级。第一轮目标是补稳第二级、稳定第三级，并开始积累第四级的实验证据。

后四个环节也有索引：考试与费曼纠错、速查表由每个单元的[固定闭环](../docs/00_learning_system/learning_map.md#每个单元的固定闭环)承接，再按[间隔复习计划](../docs/00_learning_system/review_schedule.md)进入[项目验证](../ROADMAP.md)。

为了让这套流程可执行，我还会把诊断模式和教学模式明确分开。
...
五级学习阶梯描述能力深度，十个学习单元描述第一轮的内容与时间顺序；两者不是两套独立计划。
...
可复制的不是我的定位建图题目，而是这套闭环：

1. 把“大主题”改写成可解释、可计算、可实现、可判断的能力；
2. 提供背景和目标，让 ChatGPT 闭卷诊断，一次一题，并保留原始回答；
3. 限制每个模块的题数、总时长和停止条件；
4. 根据错误重排学习顺序，已掌握部分只做综合验证；
5. 每个主题留下独立复述、公式说明、推导或实验、系统位置与故障分析等证据；
6. 用延迟测试和真实项目复验，错误不再复发才算形成稳定能力。

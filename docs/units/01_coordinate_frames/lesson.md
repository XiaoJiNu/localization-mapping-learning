...
配图统一使用以下颜色：蓝色表示坐标系 $A$ 或目标表达，橙色表示坐标系 $B$ 或源表达，绿色表示平移，紫红色表示物理点或需要特别关注的量。
...
![同一个物理点在两个坐标系中得到不同坐标读数](assets/lesson/01_same_point_two_frames.svg)

图中蓝色是坐标系 $A$，橙色是坐标系 $B$。两个坐标系看到的是同一个点 $P$，只是测量原点不同。
...
![点受平移影响而方向向量不受平移影响](assets/lesson/02_point_vs_vector.svg)

左图说明点依赖坐标系原点；右图说明方向向量的两个端点一起平移后，平移量会相消。
...
![旋转和平移组成点坐标变换的向量首尾相接关系](assets/lesson/03_rpt_vector_decomposition.svg)

绿色向量先从 $O_A$ 走到 $O_B$，橙色向量再从 $O_B$ 走到 $P$；两段相加得到蓝色虚线表示的最终向量。
...
第二段向量现在用 $B$ 表达，不能直接和用 $A$ 表达的平移向量相加。
...
![旋转矩阵两列分别表示源坐标系两根坐标轴的方向](assets/lesson/04_rotation_matrix_axes.svg)

图中橙色和紫色箭头分别对应旋转矩阵的第一列和第二列。矩阵的列不是抽象数字，而是坐标轴在目标坐标系中的方向。
...
![齐次坐标末位为一时平移打开末位为零时平移关闭](assets/lesson/05_homogeneous_switch.svg)

点的末位为 1，平移列参与计算；方向向量的末位为 0，平移列被自动关闭。
...
![定位系统中 lidar base odom map 四层坐标系及其职责](assets/lesson/09_localization_frames.svg)

`lidar` 安装在 `base` 上，`base` 在 `odom` 中连续运动，`map` 负责全局一致性和全局修正。
...
![测量产生时刻与处理时刻不同导致动态坐标变换误差](assets/lesson/10_time_alignment.svg)

传感器数据在 $t_0$ 产生，却在 $t_1$ 才被处理。动态变换必须查询 $t_0$，直接使用 $t_1$ 的最新位姿会引入运动误差。
